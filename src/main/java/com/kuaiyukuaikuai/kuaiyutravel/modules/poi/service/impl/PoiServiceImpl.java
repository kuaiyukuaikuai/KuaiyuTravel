...
public class PoiServiceImpl extends ServiceImpl<PoiMapper, Poi> implements PoiService {
     ...
     */
    @Override
    public Poi queryPoiById(Long id) {
        // ================== 1. 布隆过滤器前置拦截 ==================
        // 获取布隆过滤器
        ...
        if (!bloomFilter.contains(id)) {
            // 布隆过滤器判定不存在则直接返回，避免无效查询穿透到数据库
            log.warn("触发布隆过滤器拦截，恶意查询不存在的景点 ID: {}", id);
            throw new BusinessException(ErrorCode.POI_NOT_FOUND, "您查询的景点不存在！");
        }
        ...
        if (poi == null) {
            throw new BusinessException(ErrorCode.POI_NOT_FOUND, "地点不存在！");
        }
    ...
    public void update(Poi poi) {
        Long id = poi.getId();
        if (id == null) {
            throw new BusinessException(ErrorCode.PARAM_ERROR, "地点id不能为空");
        }
        // 1.更新数据库
        updateById(poi);
        // 2.删除缓存
     ...
     */
    @Override
    public List<Poi> queryPoiByType(Integer typeId, Integer current, Double x, Double y) {
        // 1.判断是否需要根据坐标查询
        if (x == null || y == null) {
            // 不需要坐标查询，按数据库查询
            // 根据类型分页查询，添加排序规则
        ...
        // 5.根据id查询poi（使用Java排序替代SQL拼接，避免注入风险）
        List<Poi> pois = query().in("id", ids).list();
        Map<Long, Poi> poiMap = pois.stream().collect(Collectors.toMap(Poi::getId, p -> p));
        List<Poi> sortedPois = new ArrayList<>(ids.size());
        for (Long id : ids) {
            Poi poi = poiMap.get(id);
            if (poi != null) {
                Distance distance = distanceMap.get(id.toString());
                if (distance != null) {
                    poi.setDistance(distance.getValue());
                }
                sortedPois.add(poi);
            }
        }
    ...
    public void savePoiWithBloomFilter(Poi poi) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SERVER_ERROR, "系统繁忙，保存失败");
        }
    ...
    public void savePoiBatchWithBloomFilter(List<Poi> poiList) {
        ...
        try {
            ...
            throw new BusinessException(ErrorCode.SERVER_ERROR, "系统繁忙，批量保存失败");
        }
    }
}
