...
public class PoiCommentServiceImpl extends ServiceImpl<PoiCommentMapper, PoiComment> implements PoiCommentService {
    ...
    public void saveComment(PoiComment poiComment) {
        // 1. 获取当前登录用户
    ...
    @Override
    public Page<PoiCommentVO> queryCommentPage(Long poiId, Integer current, Integer size) {
        // 1. 健壮性校验复用全局常量
        ...
        if (records.isEmpty()) {
            Page<PoiCommentVO> emptyPage = new Page<>(page.getCurrent(), page.getSize(), page.getTotal());
